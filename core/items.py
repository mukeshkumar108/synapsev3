...
async def _fetch_fact_links_for_fact(database: Database, *, user_id: str, from_fact_id: UUID) -> list[dict[str, Any]]:
    if hasattr(database, "fact_links"):
        ...
        return [
            row for row in rows
            if row["user_id"] == user_id and row["from_fact_id"] == from_fact_id and row.get("is_active", True)
        ]
    return await _db_fetch(
        database,
        """
...
async def record_habit_completion(
    ...
    *,
    ...
    await _insert_timeline_event(
        ...
        related_fact_ids=[fact["id"]],
        confidence=fact["confidence"],
        occurred_at=_to_naive(completed_at),
    )
    return _canonical_item(source_kind="confirmed_fact", row=fact, item_class="action")
...
async def _convert_thread_to_fact(
    ...
    *,
    ...
    existing = await _fetch_fact_by_idempotency_key(database, user_id=user_id, idempotency_key=idempotency_key, writer="frontend_manual")
    if existing:
        created_item = _frontend_manual_response(existing)
    else:
        source_uuid = _uuid_from_ref(source_ref, fallback=idempotency_key)
        ...
        created_item = _frontend_manual_response(row)
    ...
    thread["last_seen_at"] = _now_naive()
    ...
    _refresh_operational_row(thread, item_type="thread", source_id=thread["source_ids"][0] if thread.get("source_ids") else None, source_ids=thread.get("source_ids"))
    ...
    *,
    ...
    if idempotency_key and created_from.get("last_mutation_idempotency_key") == idempotency_key:
        return {
            ...
            "links": [_serialize_fact_link(link) for link in await _fetch_fact_links_for_fact(database, user_id=user_id, from_fact_id=item_id)],
        }
    if idempotency_key:
        created_from["last_mutation_idempotency_key"] = idempotency_key
    created_from["writer"] = "frontend_manual"
...
async def explain_thread(database: Database, *, user_id: str, item_id: UUID) -> dict[str, Any]:
    ...
    facts_by_id = {
        row["id"]: row
        for row in await _fetch_confirmed_facts(database, user_id=user_id)
    }
    timeline = [event for event in getattr(database, "timeline_events", []) if event["user_id"] == user_id] if hasattr(database, "timeline_events") else await _db_fetch(database, "SELECT * FROM timeline_events WHERE user_id = $1", user_id)
    ...
    for link in links:
        if link.get("to_fact_id") and link["to_fact_id"] in facts_by_id:
            ...
            linked_facts.append({
                "id": str(linked["id"]),
                "kind": _kind_for_row(linked),
                "title": (linked.get("content") or {}).get("title"),
                "status": _fact_item_status(linked),
    ...
    why_open = (
        structured_thread.get("resolution_condition")
        or metadata.get("last_status")
        or structured_thread.get("surface_reason")
        or fact.get("content", {}).get("summary")
    )
    why_now = (
        structured_thread.get("surface_reason")
        or (
            f"Snooze expired at {_to_iso(fact.get('snoozed_until'))}."
            if fact.get("snoozed_until") and not _thread_snoozed(fact)
            else "This thread is still open and marked for follow-up."
        )
    )
    return {
        "id": str(fact["id"]),
